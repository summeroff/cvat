...
export function implementJob(Job: typeof JobClass): typeof JobClass {
    ...
    Object.defineProperty(Job.prototype, 'objects', {
        value: async function objectsImplementation(): Promise<{ objects: number, attributes: number, per_label: { [key: string]: any } }> {
            ...
            const labels: { [key: string]: { objects: number, attributes: number, true_attributes: number, label_name: string, true_attributes_sums: { [specId: string]: { count: number, name: string } } } } = {};
            ...
            for (const shape of rawAnnotations.shapes) {
                ...
                labels[shape.label_id].attributes += shape.attributes.length;
                ...
                for (const attribute of shape.attributes) {
                    ...
                    if (value === 'true') {
                        ...
                        labels[shape.label_id].true_attributes_sums[spec_id].count++;
                    }
                }
            }

            const shapesAttributesSum = rawAnnotations.shapes.reduce((acc, shape) => acc + shape.attributes.length, 0);
