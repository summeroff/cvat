...
function JobItem(props: Props): JSX.Element {
    const {
        job, task, onJobUpdate, childJobs, defaultCollapsed, onCollapseChange, jobDataArray, addObject } = props;
    ...
    const childJobViews: React.JSX.Element[] = childJobs ? childJobs.sort((a, b) => a.id - b.id)
        .map((eachJob: Job) => (
            <JobItem key={eachJob.id} job={eachJob} task={task} onJobUpdate={onJobUpdate} jobDataArray={undefined} addObject={function (newData: JobData): void {
    ...
    const compact_return = (
        <Col span={24}>
            <Card className='cvat-job-item-compact' style={{ ...style }} data-row-id={job.id}>
                <Row align='middle'>
                    <Col span={4}>
                        <Row>
                            <Col>
                    ...
                    <Col span={3}>
                        ...
                        >
                            ...
                            <Select.Option value={JobStage.VALIDATION}>{JobStage.VALIDATION}</Select.Option>
                    ...
                    <Col span={5}>
                        <Text>
                            ...
                            {job.type !== JobType.GROUND_TRUTH && ` | ${job.startFrame}-${job.stopFrame}`}
                        </Text>
                    </Col>
                    <Col span={4}>
                        <Text>Objects: </Text>{<LabelingSummaryComponent jobInstance={job} jobDataArray={jobDataArray} addObject={addObject} />}
                    </Col>
                    <Col span={1} style={{ textAlign: 'right' }}>
                        <JobActionsComponent
                            jobInstance={job}
                            consensusJobsPresent={(childJobs as Job[]).length > 0}
                            triggerElement={<MoreOutlined className='cvat-job-item-more-button' />}
                        />
                    </Col>
                </Row>
                {childJobViews.length > 0 && (
